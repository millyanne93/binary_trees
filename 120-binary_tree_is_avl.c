...
int binary_tree_is_avl(const binary_tree_t *tree)
{
	...
	return (avlCheckRecursive(tree, INT_MIN, INT_MAX));
}

/**
 * avlCheckRecursive - performs the AVL check recursively
 * @tree: pointer to the root node of the tree
 * @min: minimum value
 * @max: maximum value
 *
...
int avlCheckRecursive(const binary_tree_t *tree, int min, int max)
{
	...
	if (abs(path_l - path_r) > 1)
		return (0);
	return (avlCheckRecursive(tree->left, min, tree->n - 1) &&
			avlCheckRecursive(tree->right, tree->n + 1, max));
}
